...
describe('UpsertCompanyPostgresRepository', () => {
  ...
  test('Should call update with federativeEntity as an empty string if it was undefined', async () => {
    const sut = makeSut();
    ...
    const query: string = dbMock.mock.calls[1].join('').trim().replace(/\\n/gms, '');

    const updateFields = (/UPDATE .+ SET (.+) WHERE .+[;]$/gms.exec(query) as RegExpExecArray)[1];

    const federativeEntityPosition = updateFields
      .replace(/[";\n ]/g, '')
      .replace(/([ = ,, ]+|[ = ,])/g, ',')
      .split(',')
      .indexOf('federativeEntity');
